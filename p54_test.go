...
type p54PokerHand struct {
	cards     [5]Card
	multiples map[int]int
}

// sort.Interface
func (h *p54PokerHand) Len() int { return 5 }
func (h *p54PokerHand) Less(i, j int) bool {
	return h.cards[i].Value() > h.cards[j].Value()
}
func (h *p54PokerHand) Swap(i, j int) {
	h.cards[i], h.cards[j] = h.cards[j], h.cards[i]
}

func p54NewPokerHand(s string) *p54PokerHand {
	h := new(p54PokerHand)
	h.multiples = make(map[int]int)
	for i, sc := range strings.Split(s, " ") {
		h.cards[i][0] = sc[0]
		h.cards[i][1] = sc[1]
		h.multiples[h.cards[i].Value()] += 1
	}
	sort.Sort(h)
	return h
}

func (h *p54PokerHand) HighestCard() int {
	return h.cards[0].Value()
}

func (h *p54PokerHand) IsOnePair() int {
	for v, m := range h.multiples {
		if m == 2 {
			return 200 + v
		}
	}
...
func (h *p54PokerHand) IsTwoPairs() int {
	pairs := make([]int, 0, 2)
	for v, m := range h.multiples {
		if m == 2 {
...
func (h *p54PokerHand) IsThreeOfAKind() int {
	for v, m := range h.multiples {
		if m == 3 {
			return 400 + v
		}
...
func (h *p54PokerHand) IsStraight() int {
	v := h.cards[0].Value()
	for idx, c := range h.cards[1:] {
		if c.Value() != v-idx-1 {
			return 0
		}
	}
...
func (h *p54PokerHand) IsFlush() int {
	v := h.cards[0][1]
...
func (h *p54PokerHand) IsFullHouse() int {
	st := h.IsThreeOfAKind()
	if st == 0 {
		return 0
	}
...
func (h *p54PokerHand) IsFourOfAKind() int {
	for v, m := range h.multiples {
		if m == 4 {
			return 800 + v
		}
...
func (h *p54PokerHand) IsStraightFlush() int {
	ss := h.IsStraight()
	if ss == 0 {
		return 0
	}
...
func (h *p54PokerHand) IsRoyalFlush() int {
	if h.cards[0][0] == 'A' && h.IsStraightFlush() > 0 {
		return 1000
	}
	return 0
}

func (h *p54PokerHand) Score() int {
	if sc := h.IsRoyalFlush(); sc != 0 {
		return sc
	}
	if sc := h.IsStraightFlush(); sc != 0 {
		return sc
	}
	if sc := h.IsFourOfAKind(); sc != 0 {
		return sc
	}
	if sc := h.IsFullHouse(); sc != 0 {
		return sc
	}
	if sc := h.IsFlush(); sc != 0 {
		return sc
	}
	if sc := h.IsStraight(); sc != 0 {
		return sc
	}
	if sc := h.IsThreeOfAKind(); sc != 0 {
		return sc
	}
	if sc := h.IsTwoPairs(); sc != 0 {
		return sc
	}
	if sc := h.IsOnePair(); sc != 0 {
		return sc
	}
	return h.HighestCard()
}

func (h *p54PokerHand) CardValues() int {

	v := h.cards[0].Value() * 20 * 20 * 20 * 20
	v += h.cards[1].Value() * 20 * 20 * 20
	v += h.cards[2].Value() * 20 * 20
	v += h.cards[3].Value() * 20
	v += h.cards[4].Value()

	return v
}

func p54Winner(h1, h2 *p54PokerHand) int {
	sc1 := h1.Score()
	sc2 := h2.Score()
	if sc1 > sc2 {
		return 1
	...
	if h1.CardValues() > h2.CardValues() {
		return 1
	} else {
		return 2
	}
...
func TestP54(t *testing.T) {
	...
	h1 := p54NewPokerHand("5H 5C 6S 7S KD")
	h2 := p54NewPokerHand("2C 3S 8S 8D TD")
	if h1.Score() != 205 || h2.Score() != 208 {
		t.Error(h1.Score(), h2.Score())
	}
	if p54Winner(h1, h2) != 2 {
		t.Error(h1, h2)
	}

	h1 = p54NewPokerHand("5D 8C 9S JS AC")
	h2 = p54NewPokerHand("2C 5C 7D 8S QH")
	if h1.Score() != 14 || h2.Score() != 12 {
		t.Error(h1.Score(), h2.Score())
	}
	if p54Winner(h1, h2) != 1 {
		t.Error(h1, h2)
	}

	h1 = p54NewPokerHand("2D 9C AS AH AC")
	h2 = p54NewPokerHand("3D 6D 7D TD QD")
	if h1.Score() != 414 || h2.Score() != 600 {
		t.Error(h1.Score(), h2.Score())
	}
	if p54Winner(h1, h2) != 2 {
		t.Error(h1, h2)
	...
	h1 = p54NewPokerHand("4D 6S 9H QH QC")
	h2 = p54NewPokerHand("3D 6D 7H QD QS")
	if h1.Score() != 212 || h2.Score() != 212 {
		t.Error(h1.Score(), h2.Score())
	}
	if p54Winner(h1, h2) != 1 {
		t.Error(h1, h2)
	...
	h1 = p54NewPokerHand("2H 2D 4C 4D 4S")
	h2 = p54NewPokerHand("3C 3D 3S 9S 9D")
	if h1.Score() != 704 || h2.Score() != 703 {
		t.Error(h1.Score(), h2.Score())
	}
	if p54Winner(h1, h2) != 1 {
		t.Error(h1, h2)
	...
	p1win := 0
	for _, line := range strings.Split(p54hands, "\n") {
		if len(line) == 0 {
			continue
		}

		h1 := p54NewPokerHand(line[0:14])
		h2 := p54NewPokerHand(line[15:])
